...
def generate_plot_data(log_results, sketch_method, mode='ip'):
    ...
    for key in log_results:
        ...
        try:
            ip, corr, _, ip_scale = log_results[key]['true']
        except:
            ip, corr, _ = log_results[key]['true']
            vecA, vecB = log_results['vecA'], log_results['vecB']
            (ip_scale, _, _, _, _, _) = get_scale(vecA, vecB)

        if sketch_method not in log_results[key]:
            continue
...
def extract_from(data_file):
    ...
    # Extract the values
    overlap = float(name_splits[1].split('_')[1])
    outlier = float(name_splits[2].split('_')[1])
    corr = float(name_splits[4].split('_')[1])
    mode = name_splits[0].split('_')[1]
