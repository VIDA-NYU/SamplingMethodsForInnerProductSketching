...
class DartMHSketch(InnerProdSketch):
    def __init__(self, sk_hashes: np.ndarray, sk_values: np.ndarray, vec_l2: float) -> None:
    ...
    @staticmethod 
    @njit(parallel=False)
    def inner_product_numba(sk_hashesA, sk_valuesA, sk_hashesB, sk_valuesB, vecA_l2, vecB_l2):
        m = len(sk_hashesA)
        sum_min = 0.0
        ...
        wmh_est = vecA_l2*vecB_l2*(M_est/m)*sum_k
        return wmh_est

    def inner_product(self, other: 'DartMHSketch') -> float:
        t1 = time.time()
        res = self.inner_product_numba(self.sk_hashes, self.sk_values, other.sk_hashes, other.sk_values, self.vec_l2, other.vec_l2)
        t2 = time.time()
...
class DartMH(InnerProdSketcher):
    ...
    def sketch(self, vector: np.ndarray) -> DartMHSketch:
        ...
        return DartMHSketch(sk_hashes, sk_values, vec_l2)